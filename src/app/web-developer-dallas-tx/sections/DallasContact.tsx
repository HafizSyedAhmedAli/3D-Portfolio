...
import { dallasAvailability } from "@/constants/dallas";
...
const DallasContact = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!serviceId || !templateId || !publicKey) {
      setFeedback({ type: "error", message: "Configuration error — please reach me directly." });
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      setForm({ name: "", email: "", phone: "", budget: "", city: "", message: "" });
      setFeedback({ type: "success", message: "Message delivered. I'll be in touch soon." });
    } catch {
      setFeedback({ type: "error", message: "Delivery failed — try reaching me directly." });
    } finally {
      setLoading(false);
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-3 md:mb-4">
          Hire a Web Developer Dallas TX Today
        </h2>
        <p className="text-white/60 text-base md:text-lg leading-relaxed mb-10 md:mb-12 max-w-2xl">
          Have a project in mind? Tell me what you&apos;re building and I&apos;ll
          get back to you within 24 hours. Every engagement starts with a free
        ...
        <div className="mb-10 md:mb-12">
          ...
          <div className="rounded-2xl border border-white/8 overflow-hidden w-full">
            <iframe
              ...
              width="100%"
              height="600"
              title="Schedule a free consultation with Ahmed Ali – Web Developer Dallas TX"
              className="w-full"
            />
          </div>
        </div>
        ...
        >
          ...
          <div className="relative rounded-2xl border border-white/8 bg-white/2 overflow-hidden">
            ...
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
                <FloatInput id="dallas-name" name="name" label="Your name" value={form.name} onChange={handleChange} required disabled={loading} />
                <FloatInput id="dallas-email" name="email" label="Your email" type="email" value={form.email} onChange={handleChange} required disabled={loading} />
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
                <FloatInput id="dallas-phone" name="phone" label="Phone number" type="tel" value={form.phone} onChange={handleChange} disabled={loading} />
                <Select
                  value={form.budget}
                  onValueChange={(val) => setForm((prev) => ({ ...prev, budget: val }))}
                  disabled={loading}
                  name="budget"
                >
                  ...
                  <SelectContent className="min-w-[var(--radix-select-trigger-width)] rounded-xl border border-white/10 bg-[#0b0f0d] p-1 text-white shadow-2xl">
                    {["Under $500", "$500 – $1,000", "$1,000 – $3,000", "$3,000 – $5,000", "$5,000+"].map((b) => (
                      <SelectItem key={b} value={b} className="cursor-pointer rounded-lg px-3 py-2.5 text-sm text-white/75 focus:text-emerald-400">
                        {b}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <FloatInput id="dallas-city" name="city" label="Your city (e.g. Dallas TX)" value={form.city} onChange={handleChange} disabled={loading} />
              <FloatInput id="dallas-message" name="message" label="Project details — list all services you need…" value={form.message} onChange={handleChange} multiline required disabled={loading} />
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2 border-t border-white/6 mt-1">
                ...
                >
                  <span className="relative z-10">{loading ? "Sending…" : "Get a Free Quote →"}</span>
          ...
          <div className="flex flex-col gap-4 md:gap-5">
            <div className="rounded-xl border border-white/8 bg-white/2 p-5 md:p-6 flex flex-col gap-3 md:gap-4">
              ...
              >
                <Image src={socialImgs[0]?.imgPath} alt="" aria-hidden="true" width={20} height={20} className="grayscale opacity-40 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300" />
                WhatsApp Me
              </a>
              <a
              ...
              >
                <Image src="/images/logos/linkedin.svg" alt="" aria-hidden="true" width={20} height={20} className="grayscale opacity-40 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300" />
                LinkedIn
              </a>
            </div>

            <AvailabilityPanel items={dallasAvailability} />
          </div>
        </div>
      </div>
    </section>
