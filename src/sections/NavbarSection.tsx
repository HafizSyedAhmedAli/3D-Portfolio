  ...
  return (
    <>
      ...
      >
        <div className="inner">
          <a
            ...
            className="logo"
            onClick={close}
            aria-label="Ahmed Ali - Back to top"
          >
      ...
      <div
        onClick={close}
        aria-hidden="true"
        className={`fixed inset-0 z-[80] bg-black/60 backdrop-blur-sm transition-opacity duration-300 lg:hidden ${menuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
      />
      ...
      <nav
        ...
        className={`fixed top-0 right-0 h-full w-[280px] z-[90] bg-[#06090a] border-l border-white/8 flex flex-col pt-24 px-6 pb-8 transition-transform duration-300 ease-in-out lg:hidden ${menuOpen ? "translate-x-0" : "translate-x-full"}`}
      >
